...
connection.query = utils.promisify( db.query );
// Connect to database
const db = mysql.createConnection(
    {//help us create this centralized db or connection object that we are going to use to do all of our interaction with mySql. We need to provide it a set of configuration. Our host is always local host, usually root, then our password will be what we set it as.
      ...
      user: 'root',
      // MySQL password
      password: 'Smo1k!ng',//really important to have a specific database, not just to the server but WHICH db it should then be interacting with.  We need to insure we provide a schmema file, it's 
      database: "employees"
